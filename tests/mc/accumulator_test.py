...
def test_variance_accum_returns_single_sample():
    da = sc.DataArray(sc.arange('w', 6.0, unit='kg'),
                      coords={'h': sc.arange('w', 6) * 0.1})
    accum = VarianceAccum()
    accum.add(da)
...
def test_variance_accum_returns_number_of_samples():
    da = sc.DataArray(sc.arange('u', 3.0, unit='s'),
                      coords={'o': sc.arange('u', 3) * 0.1})
    accum = VarianceAccum()
    accum.add(da)
    accum.add(da)
    accum.add(da)
...
def test_variance_accum_preserves_metadata():
    ...
    da = sc.DataArray(sc.array(dims=['variable', 'observation'],
                               values=rng.normal(0.0, 1.0, (9, 2))),
...
def test_variance_accum_add_from():
    rng = np.random.default_rng(83)
    da = sc.DataArray(
        sc.array(dims=['observation', 'variable'],
                 values=rng.normal(0.0, 1.0, (14, 8))))
...
def test_covariance_accum_returns_expected_result():
    ...
    da = sc.DataArray(
        sc.array(dims=['variable', 'observation'],
                 values=rng.normal(0.0, 1.0, (5, 19))))
...
def test_covariance_accum_returns_number_of_samples():
    ...
    da = sc.DataArray(
        sc.array(dims=['variable'], values=rng.normal(0.0, 1.0, 6)))
